/**
 * libusbip - idev_serial.c (Prints the serial string of an attached iDevice.)
...
idevice_available(struct libusbip_connection_info *ci, libusbip_ctx_t ctx,
    ...
    for (i = 0; i < max; i++) {
        ...
        if (dd.idProduct == 0x1280
         ...
         || dd.idProduct == 0x1283
         || dd.idProduct == 0x1227) {
            printf("[*] idev_serial: found iDevice in recovery mode (%04x:%04x %u)\n",
...
main(int argc, char *argv[]) {
    ...
    if (argc < 3) {
        printf("idev_serial: <server-ip> <server-port>\n");
        goto fail;
    }
    ...
    if (!data) {
        printf("[*] idev_serial: malloc failed\n");
        goto release_fail;
    ...
    printf("[*] idev_serial: found serial string (%s)\n", data);
    free(data);
